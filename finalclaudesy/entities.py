...
class Character:
    ...
    @property
    def max_hp(self): return self.base['hp'] + self.growth['hp'] * (self.level - 1)
    @property
    def max_mp(self): return self.base['mp'] + self.growth['mp'] * (self.level - 1)
    @property
    def atk(self):    return int((self.base['atk'] + self.growth['atk']*(self.level-1) + self._equip('atk')) * self.buffs.get('atk', 1.0))
    @property
    def defense(self):
    ...
    @property
    def mag(self):    return int((self.base['mag'] + self.growth['mag']*(self.level-1) + self._equip('mag')) * self.buffs.get('mag', 1.0))
    @property
    def spd(self):    return int((self.base['spd'] + self.growth['spd']*(self.level-1) + self._equip('spd')) * self.buffs.get('spd', 1.0))

    @property
    def hp(self): return self._hp
    @hp.setter
    def hp(self, v): self._hp = max(0, min(self.max_hp, v))

    @property
    def mp(self): return self._mp
    @mp.setter
    def mp(self, v): self._mp = max(0, min(self.max_mp, v))
...
class EnemyInstance:
    ...
    @property
    def defense_eff(self):
        v = self.defense * self.buffs.get('def', 1.0)
        return int(v * 1.5 if self.defending else v)

    @property
    def atk_eff(self):  return int(self.atk * self.buffs.get('atk', 1.0))
    @property
    def mag_eff(self):  return int(self.mag  * self.buffs.get('mag', 1.0))
...
class Party:
    def __init__(self) -> None:
        ...
        self.map_x     = 13
        self.map_y     = 7
        # NewGame+ flag — set after defeating SYNTHOS once. Carries over to a
        # new playthrough; enemies scale and a bonus boss becomes available.
        self.newgame_plus = False
    ...
    def save_dict(self) -> dict:
        return {
            ...
            'map_x':         self.map_x,
            'map_y':         self.map_y,
            'newgame_plus':  self.newgame_plus,
        }
    ...
    def from_dict(cls, d: dict) -> 'Party':
        ...
        p.newgame_plus   = bool(d.get('newgame_plus', False))
        return p

    def start_newgame_plus(self) -> 'Party':
        """Carry levels and equipment into a fresh world. Story flags reset so
        bosses can be re-fought; gold halved; KO members revived; bonus-boss
        flag enabled."""
        for m in self.members:
            m.hp = m.max_hp
            m.mp = m.max_mp
            m.status.clear()
        self.gold = max(400, self.gold // 2)
        self.story_flags = {'newgame_plus_started'}
        self.dungeon_done = set()
        self.map_x = 13
        self.map_y = 7
        self.newgame_plus = True
        return self
