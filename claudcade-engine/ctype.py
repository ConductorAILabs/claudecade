...
def draw_how_to_play(scr, H, W, tick):
    _engine_how_to_play(
        ...
        tips=[
            ...
            '• Power-ups restore a shot type',
            '• Boss appears every 6 waves',
        ],
    )
FPS        = 30
AT         = 3          # arena top row
...
# ── Boss frames ───────────────────────────────────────────────────────────────
# Phase 0 (full HP) — cold, mechanical, intimidating
# Phase 1 (<50% HP) — damaged, fires `!!!`, glows magenta
BOSS_FRAMES = [
    [   # frame A — calm
        '  ╔══◆══╗  ',
        ' ╔╝▓▓█▓▓╚╗ ',
...
class Boss:
    ...
    def sprites(self):
        # Phase 0 → frames 0,1 (calm blue-green); Phase 1 → frames 2,3 (rage magenta)
...
def draw_game(scr, game, H, W, tick):
    ...
    for e in game.enemies:
        ...
        # Color per type: grunt=red, sine=magenta, diver=cyan, heavy=green, turret=yellow
    ...
    if game.boss and game.boss.alive:
        ...
        # Phase 0: red; Phase 1 (enraged): magenta + blink alternate
        if game.boss.phase == 0:
            cp = P(2)|curses.A_BOLD
        else:
            cp = (P(6) if (tick//4)%2==0 else P(2))|curses.A_BOLD
        for i,row in enumerate(rows):
            r=br+i
