...
import claudcade_engine as e  # noqa: E402  (sys.path setup must precede import)
...
def _stub_curses_for_headless():
    """curses.color_pair() requires initscr(); stub it so Renderer can run headless."""
    curses.color_pair = lambda n: 0
    for attr in ("A_BOLD", "A_DIM", "A_REVERSE", "A_BLINK"):
...
def test_engine_helpers():
    """Pytest entrypoint — exercises every engine helper and asserts no failures."""
    failures: list[str] = []
    ...
    _run_all(check)
    assert not failures, "engine helper failures:\n  - " + "\n  - ".join(failures)
