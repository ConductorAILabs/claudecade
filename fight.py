...
class Fighter:
    ...
    def update(self, other, keys=None, mp=False, mk=False):
        ...
        self.physics(other)
COMBO_WINDOW = 60   # frames (~2s at 30fps) before a combo resets

class Game:
    def __init__(self, p1_char, p2_char, stage_idx=0):
    ...
    def _hits(self):
        for a,d,who in [(self.p1,self.p2,'p1'),(self.p2,self.p1,'p2')]:
            if a.hit_active() and not a.hit_dealt:
                ...
                if abs(d.x-a.x)<=rng and d.can_be_hit_by(a.state):
                    ...
                    if actual:
                        ...
                        c = self.combo[who]
                        c['n'] = c['n'] + 1 if c['t'] > 0 else 1
                        c['t'] = COMBO_WINDOW
                        c['peak'] = max(c['peak'], c['n'])
    ...
    def update(self, keys, mp=False, mk=False):
        ...
        # Decay combo timers; reset counter when window expires.
        for c in self.combo.values():
            if c['t'] > 0:
                c['t'] -= 1
                if c['t'] == 0: c['n'] = 0
...
def draw_fight(scr, g, W, H):
    ...
    # ── Combo counters (above each fighter) ──────────────────────────────────
    for who, fighter, color in (('p1', g.p1, g.p1.cp), ('p2', g.p2, g.p2.cp)):
        c = g.combo[who]
        if c['n'] >= 2 and c['t'] > 0:
            tag = f' {c["n"]} HIT COMBO! '
            col = 1 + int(fighter.x / ARENA_W * (W - len(tag) - 2))
...
class IntroScene(Scene):
    def update(self, inp, tick, dt):
        if inp.pressed(ord(' ')): return ('select', None)
        if inp.pressed(27):       return 'quit'
...
class SelectScene(Scene):
    def on_enter(self):
        # Payload: int (preferred selection) when re-entering after a match.
        if isinstance(self.payload, int):
            self.sel = self.payload % len(CHARS)
        elif not hasattr(self, 'sel'):
            self.sel = 0

    def update(self, inp, tick, dt):
        if inp.just_pressed(curses.KEY_LEFT):  self.sel = (self.sel - 1) % len(CHARS)
        if inp.just_pressed(curses.KEY_RIGHT): self.sel = (self.sel + 1) % len(CHARS)
        if inp.just_pressed(ord(' ')):
            return ('howtoplay', self.sel)
        if inp.pressed(27): return 'quit'

    def draw(self, r, tick):
        draw_select(r._scr, self.engine.H, self.engine.W, self.sel, tick)
...
# Stages: visual variation only. Floor pattern + name + color.
STAGES = [
    {'name': 'DOJO',         'floor': '═', 'cp': 4, 'subtitle': 'Wood and discipline'},
    {'name': 'MOUNTAIN',     'floor': '▓', 'cp': 3, 'subtitle': 'Cold air, sharp rock'},
    {'name': 'NEON CITY',    'floor': '▒', 'cp': 6, 'subtitle': 'Distant signs flicker'},
    {'name': 'OUTPOST',      'floor': '─', 'cp': 1, 'subtitle': 'Recycled steel deck'},
]
...
class StageSelectScene(Scene):
    def on_enter(self):
        self.p1_char = self.payload if isinstance(self.payload, int) else 0
        if not hasattr(self, 'sel_stage'):
            self.sel_stage = 0

    def update(self, inp, tick, dt):
        if inp.just_pressed(curses.KEY_LEFT):  self.sel_stage = (self.sel_stage - 1) % len(STAGES)
        if inp.just_pressed(curses.KEY_RIGHT): self.sel_stage = (self.sel_stage + 1) % len(STAGES)
        if inp.just_pressed(ord(' ')):
            return ('countdown', (self.p1_char, self.sel_stage))
        if inp.pressed(27): return 'quit'
    ...
    def draw(self, r, tick):
        ...
        r.text(2, 0, '╠'+'═'*(W-2)+'╣', curses.color_pair(5)|curses.A_BOLD)
        bw = 18; gap = 4
        total = len(STAGES) * (bw + gap) - gap
        start = max(2, (W - total) // 2)
        for i, st in enumerate(STAGES):
            ...
            side = '║' if sel else '│'
            box_y = max(4, H // 2 - 4)
            r.text(box_y, cx, top, st['cp'] if sel else 5, bold=sel)
            for row in range(1, 6):
                line = side + (st['floor']*(bw-2)) + side if row == 5 else side + ' '*(bw-2) + side
                r.text(box_y+row, cx, line, st['cp'] if sel else 5, bold=sel)
                if row == 2:
                    r.text(box_y+row, cx + (bw-len(st['name']))//2, st['name'],
                           st['cp'] if sel else 5, bold=True)
                if row == 3:
                    r.text(box_y+row, cx+1, st['subtitle'][:bw-2].center(bw-2), 5, dim=True)
            r.text(box_y+6, cx, bot, st['cp'] if sel else 5, bold=sel)
    ...
    def update(self, inp, tick, dt):
        ...
        if self.timer <= 0:
            if self.phase > 0:
                ...
                p2_choices = [i for i in range(len(CHARS)) if i != self.p1_char]
                p2_char    = random.choice(p2_choices)
                return ('fight', (self.p1_char, p2_char, self.stage_idx))

    def draw(self, r, tick):
        draw_countdown(r._scr, self.engine.H, self.engine.W, self.phase)
...
class FightScene(Scene):
    ...
    def update(self, inp, tick, dt):
        ...
        mp = 1 in inp.mouse_pressed
        mk = 3 in inp.mouse_pressed
        self.game.update(inp.keys, mp, mk)
        if self.game.over and inp.just_pressed(ord('\n'), 10, 13):
            if not self.game.next_round():
                extra = f'{"Win" if self.game.p1w >= NEED else "Loss"} {self.game.p1w}-{self.game.p2w}'
                submit_async('fight', self.game.p1w * 100, extra, [None])
                return ('select', self.p1_char)
            # next round: re-enter countdown with same chars + stage
            return ('countdown', (self.p1_char, self.stage_idx))

    def draw(self, r, tick):
        draw_fight(r._scr, self.game, self.engine.W, self.engine.H)
        if self.paused:
            draw_pause(r._scr, self.engine.H, self.engine.W)


def main():
    Engine('Claude Fighter', fps=FPS) \
        .scene('intro',      IntroScene()) \
        .scene('select',     SelectScene()) \
        .scene('howtoplay',  HowToPlayScene()) \
        .scene('stage',      StageSelectScene()) \
